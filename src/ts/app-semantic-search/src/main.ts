...
function cosineSimilarity(vector1: number[], vector2: number[]): number {
  ...
  if (magnitude1 === 0 || magnitude2 === 0) {
    throw new Error(
      "Magnitude of a vector must be non-zero for cosine similarity calculation."
    );
  }
...
async function main() {
  try {
    ...
    const parrotEmbedding = await client.getEmbeddings(deploymentName, [
      parrot,
    ]);
    const embeddings = await client.getEmbeddings(deploymentName, [source]);
    const embeddingsCompareTo = await client.getEmbeddings(deploymentName, [
      compareTo,
    ]);
    ...
    console.log(
      "Comparing - Car vs Vehicle...: ",
      scoreCarWithVehicle.toFixed(7)
    );

    const scoreCarWithParrot = cosineSimilarity(carArray, parrotArray);
    console.log(
      "Comparing - Car vs Parrot...: ",
      scoreCarWithParrot.toFixed(7)
    );
  } catch (error) {
